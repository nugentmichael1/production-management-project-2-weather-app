    ...
    while not stop_event.is_set():
        try:
            ...
            if response.status_code == 200:
                ...
                logging.info(f"Weather data updated for {weather_data['city']}")

            else:
                logging.warning(f"Failed to fetch weather data: {response.status_code} for lat {lat}, lon {lon}")
                
        except requests.exceptions.RequestException as e:
            logging.error(f"Weather fetch error: {e}")
        
        except Exception as e:
            logging.critical(f"Unexpected error in weather thread: {e}", exc_info=True)
...
def start_weather_threads():
    ...
    for city, coords in CITIES.items():
        ...
        logging.info(f"Started weather thread for {city}")
    
def stop_all_weather_threads():
    """Stops all weather threads."""
    for city, thread in weather_threads.items():
        logging.info(f"Stopping weather thread for {city}")
    ...
    logging.info("All weather threads stopped.")